    ...
    @section('content')
    <style>
    /* page-scoped styles to match app's luxury aesthetic */
    .books-hero {
        display:flex;
        align-items:center;
    ...
    .btn-pill.danger {
        background:transparent;
        border-color:rgba(220,40,40,0.12);
    ...
    <div class="container">
        @if ($message = Session::get('success'))
            <div class="alert" style="background:#e8ffe8;border:1px solid #bce8bc;color:#042; border-radius:12px;padding:10px 14px;margin-bottom:14px;">
                {{ $message }}
            </div>
        @endif
                    ...
                    <article class="book-card" aria-labelledby="book-{{ $book->id }}">
                        ...
                        <div class="card-meta">
                            <div class="actions" role="group" aria-label="book actions">
                                <a href="{{ route('books.show', $book->id) }}" class="pill-action show" title="Show">
                                    Show
                                </a>

                                @can('edit-books')
                                    <a href="{{ route('books.edit', $book->id) }}" class="pill-action edit" title="Edit">
                                        Edit
                                    </a>
                                @endcan
                                ...
                                @can('delete-books')
                                    <form action="{{ route('books.destroy', $book->id) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="pill-action delete" onclick="return confirmDelete(this);" title="Delete">
                                            Delete
                                        </button>
                                    </form>
                                @endcan
    ...
    function confirmDelete(btn){
        // small unobtrusive confirm - return true to proceed
