...
@section('content')
<style>
.roles-hero{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:1.25rem}
.roles-hero h2{margin:0;font-weight:400;letter-spacing:-0.3px}
...
.role-card{background:#fff;border-radius:14px;padding:1.1rem 1.2rem;border:1px solid #eee;transition:transform .18s ease, box-shadow .18s ease;display:flex;flex-direction:column;min-height:150px}
.role-card:hover{transform:translateY(-6px);box-shadow:0 12px 30px rgba(0,0,0,0.08);border-color:#000}
.role-name{margin:0;font-size:1.1rem;font-weight:600}
.permissions{margin-top:10px}
.permissions span{display:inline-block;padding:4px 10px;background:#f6f6f6;border-radius:9999px;font-weight:500;margin-right:6px;margin-top:4px;font-size:.82rem}
.actions{margin-top:14px;display:flex;gap:8px}
.action-pill{padding:6px 10px;border-radius:9999px;border:1px solid transparent;background:transparent;font-weight:600;font-size:.85rem;text-decoration:none}
...
@csrf
@method('DELETE')
<button class="action-pill delete" onclick="return confirm('Delete this role?');" type="submit">Delete</button>
</form>
@endif
@endcan
@endif
...
<div class="pagination-wrapper">{{ $roles->onEachSide(1)->links() }}</div>
@else
<div class="empty">
