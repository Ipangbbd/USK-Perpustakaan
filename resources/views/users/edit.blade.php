...
@section('content')
<style>
.user-edit-hero{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:1.25rem}
.user-edit-hero h2{margin:0;font-weight:400;letter-spacing:-0.3px}
...
<label>Name</label>
<input type="text" name="name" value="{{ $user->name }}" class="lux-input @error('name') is-invalid @enderror">
@error('name')
...
<input type="email" name="email" value="{{ $user->email }}" class="lux-input @error('email') is-invalid @enderror">
@error('email')
<div class="invalid-feedback">{{ $message }}</div>
@enderror
</div>
